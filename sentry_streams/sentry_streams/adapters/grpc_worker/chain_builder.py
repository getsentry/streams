...
class ChainBuilder:
    ...
    def __init__(self) -> None:
        ...
        self.__descriptors: MutableMapping[str, Mapping[str, Any]] = {}

    def init_chain(
        self, route: Route, step_name: str, config: Mapping[str, Any], schema: Optional[str] = None
    ) -> None:
    ...
    def finalize_all(self) -> Mapping[int, ChainSegment]:
        ...
        for key, chain in self.__chains.items():
            logger.info(f"Chain {key}: {chain.steps}")

        return {i: self.__chains[route_names[i]] for i in range(len(route_names))}

    def get_descriptors(self) -> list[Mapping[str, Any]]:
        route_names = list(self.__chains.keys())
        return [self.__descriptors[route_names[i]] for i in range(len(route_names))]
